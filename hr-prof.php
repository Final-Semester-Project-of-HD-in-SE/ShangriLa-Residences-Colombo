...
if (!isset($_SESSION['username']) || !isset($_SESSION['hid'])) {
    header('Location: login.php');
    exit();
}

$hid = $_SESSION['hid'];  // Use HR ID from session
...
$name = $userData['Hname'] ?? '';
$email = $userData['Hemail'] ?? '';
$phone = $userData['Hcon'] ?? '';
$address = $userData['Hadd'] ?? '';
$profilePic = $userData['Hprof'] ?? ''; 
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['profile_pic']) && $_FILES['profile_pic']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($fileExtension, $allowedExts)) {
            $newFileName = $hid . '.' . $fileExtension; // Ensure unique file name
    ...
    $updateQuery = "UPDATE hr SET Hname = ?, Hemail = ?, Hcon = ?, Hadd = ?, Hprof = ? WHERE Hid = ?";
    $updateStmt = $connection->prepare($updateQuery);
    $updateStmt->bind_param("sssssi", $newName, $newEmail, $newPhone, $newAddress, $profilePic, $hid);
    ...
    <main>
        <section class="profile">
            <div class="container">
                ...
                <?php endif; ?>
                <form class="profile-form" action="hr-prof.php" method="post" enctype="multipart/form-data">
